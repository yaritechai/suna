        ...
        <div className="flex flex-wrap gap-1.5 max-h-20 overflow-y-auto px-3">
          {uploadedFiles.map((file, index) => (
            <motion.div
              ...
              className={cn(
                'px-2 py-1 bg-base-100 rounded-md flex items-center gap-1.5 group text-sm',
                !sandboxId ? 'border-info/20' : '',
              )}
            >
              ...
              <span className="text-xs text-base-content/70 flex-shrink-0">
                ...
                {!sandboxId && (
                  <span className="ml-1 text-info">(pending)</span>
                )}
              </span>
              <Button
                type="button"
                variant="ghost"
                size="icon"
                className="h-4 w-4 rounded-full p-0 hover:bg-base-300"
