...
class MCPManager:
    ...
    async def connect_server(self, mcp_config: Dict[str, Any]) -> MCPConnection:
        ...
        if qualified_name in self.connections:
            logger.info("MCP server {qualified_name} already connected")
            return self.connections[qualified_name]

        logger.info("Connecting to MCP server: {qualified_name}")
        ...
        try:
            ...
            # Create server URL
            url = "{SMITHERY_SERVER_BASE_URL}/{qualified_name}/mcp?config={config_b64}&api_key={SMITHERY_API_KEY}"
            ...
            async with streamablehttp_client(url) as (read_stream, write_stream, _):
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    logger.info("MCP session initialized for {qualified_name}")

                    # List available tools
                    tools_result = await session.list_tools()
            tools = tools_result.tools if hasattr(tools_result, 'tools') else tools_result

            logger.info("Available tools from {qualified_name}: {[t.name for t in tools]}")
            ...
            logger.error("Failed to connect to MCP server {qualified_name}: {str(e)}")
            raise
    ...
    async def connect_all(self, mcp_configs: List[Dict[str, Any]]) -> None:
        ...
        for config in mcp_configs:
            try:
                ...
                logger.error("Failed to connect to {config['qualifiedName']}: {str(e)}")
    ...
    def get_all_tools_openapi(self) -> List[Dict[str, Any]]:
        ...
        for conn in self.connections.values():
            ...
            for tool in conn.tools:
                ...
                openapi_tool = {
                    "name": "mcp_{conn.qualified_name}_{tool.name}",  # Prefix to avoid conflicts
                    "description": tool.description or "MCP tool from {conn.name}",
                    "parameters": {
                        "type": "object",
                        "properties": {},
                        "required": []
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(parts) != 3 or parts[0] != "mcp":
            raise ValueError("Invalid MCP tool name format: {tool_name}")

        _, qualified_name, original_tool_name = parts

        # Find the connection config
        if qualified_name not in self.connections:
            raise ValueError("MCP server {qualified_name} not connected")

        conn = self.connections[qualified_name]

        logger.info("Executing MCP tool {original_tool_name} on server {qualified_name}")
    ...
    async def disconnect_all(self):
        ...
        for qualified_name in list(self.connections.keys()):
            try:
                ...
                logger.info("Cleared MCP server configuration for {qualified_name}")
            except Exception as e:
                logger.error("Error clearing configuration for {qualified_name}: {str(e)}")
