...
async def list_mcp_servers(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info("Successfully fetched {len(data.get('servers', []))} MCP servers")
            return MCPServerListResponse(**data)

    except httpx.HTTPStatusError as e:
        logger.error("HTTP error fetching MCP servers: {e.response.status_code} - {e.response.text}")
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Failed to fetch MCP servers from Smithery: {e.response.text}"
        )
    except Exception as e:
        logger.error("Error fetching MCP servers: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail="Failed to fetch MCP servers: {str(e)}"
        )
...
async def get_mcp_server_details(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            url = "{SMITHERY_API_BASE_URL}/servers/{encoded_name}"
            logger.debug("Requesting MCP server details from: {url}")
            ...
            logger.info("Successfully fetched details for MCP server: {qualified_name}")
            logger.debug("Response data keys: {list(data.keys()) if isinstance(data, dict) else 'not a dict'}")

            return MCPServerDetailResponse(**data)

    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            logger.error("Server not found. Response: {e.response.text}")
            raise HTTPException(status_code=404, detail="MCP server '{qualified_name}' not found")

        logger.error("HTTP error fetching MCP server details: {e.response.status_code} - {e.response.text}")
        raise HTTPException(
            status_code=e.response.status_code,
            detail="Failed to fetch MCP server details: {e.response.text}"
        )
    except Exception as e:
        logger.error("Error fetching MCP server details: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail="Failed to fetch MCP server details: {str(e)}"
        )
...
async def get_popular_mcp_servers_v2(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info("Successfully categorized {len(servers)} servers into {len(sorted_categories)} categories")
