...
class Configuration:
    ...
    OR_SITE_URL: Optional[str] = "https://yaritech.ai"
    OR_APP_NAME: Optional[str] = "Yaritech AI"
    ...
    # Model configuration
    MODEL_TO_USE: Optional[str] = "anthropic/claude-3-5-sonnet-latest"
    ...
    # Search and other API keys (optional for basic functionality)
    ...
    SANDBOX_ENTRYPOINT = "/usr/bin/supervisord -n -c /etc/supervisor/conf.d/supervisord.con"
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("Invalid ENV_MODE: {env_mode_str}, defaulting to LOCAL")
            self.ENV_MODE = EnvMode.LOCAL

        logger.info("Environment mode: {self.ENV_MODE.value}")
            ...
            if env_val is not None:
                ...
                if expected_type == bool:
                    ...
                    try:
                        ...
                        logger.warning("Invalid value for {key}: {env_val}, using default")
    ...
    def _validate(self):
        ...
        if missing_fields:
            error_msg = "Missing required configuration fields: {', '.join(missing_fields)}"
