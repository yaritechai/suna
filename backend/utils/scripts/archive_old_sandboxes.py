...
async def get_old_projects(days_threshold: int = 1) -> List[Dict[str, Any]]:
    ...
    logger.info("Starting to fetch projects older than {days_threshold} day(s)")
    print("Looking for projects created before: {threshold_date}")
    ...
    while has_more:
        ...
        logger.info("Fetching projects page {current_page+1} (range: {start_range}-{end_range})")
        ...
        try:
            ...
            print("Response data length: {len(result.data)}")
            ...
            if not result.data:
                ...
                if current_page == 0 and result.data:
                    print("Sample project data: {result.data[0]}")

                all_projects.extend(result.data)
                current_page += 1

                # Progress update
                logger.info("Loaded {len(all_projects)} projects so far")
                print("Loaded {len(all_projects)} projects so far...")

                # Check if we've reached the end - if we got fewer results than the page size
                if len(result.data) < page_size:
                    print("Got {len(result.data)} records which is less than page size {page_size}, ending pagination")
                    has_more = False
                else:
                    print("Full page returned ({len(result.data)} records), continuing to next page")

        except Exception as e:
            logger.error("Error during pagination: {str(e)}")
            print("Error during pagination: {str(e)}")
            has_more = False  # Stop on error

    # Print the query result summary
    total_projects = len(all_projects)
    print("Found {total_projects} total projects in database")
    logger.info("Total projects found in database: {total_projects}")
    ...
    logger.info("Found {len(old_projects_with_sandboxes)} old projects with sandboxes")
    ...
    if old_projects_with_sandboxes:
        ...
        for i, project in enumerate(old_projects_with_sandboxes[:3]):
            print("  {i+1}. {project.get('name')} (Created: {project.get('created_at')})")
            print("     Sandbox ID: {project['sandbox'].get('id')}")
            if i >= 2:
                break
...
async def archive_sandbox(project: Dict[str, Any], dry_run: bool) -> bool:
    ...
    try:
        logger.info("Checking sandbox {sandbox_id} for project '{project_name}' (ID: {project_id}, Created: {created_at})")

        if dry_run:
            logger.info("DRY RUN: Would archive sandbox {sandbox_id}")
            print("Would archive sandbox {sandbox_id} for project '{project_name}' (Created: {created_at})")
            return True
        ...
        logger.info("Sandbox {sandbox_id} is in '{sandbox_info.state}' state")

        # Only archive if the sandbox is in the stopped state
        if sandbox_info.state == "stopped":
            logger.info("Archiving sandbox {sandbox_id} as it is in stopped state")
            sandbox.archive()
            logger.info("Successfully archived sandbox {sandbox_id}")
            return True
        else:
            logger.info("Skipping sandbox {sandbox_id} as it is not in stopped state (current: {sandbox_info.state})")
            return True
        ...
        logger.error("Error processing sandbox {sandbox_id}: {str(e)}")
        logger.error("Error type: {error_type}")
        ...
        if hasattr(e, 'response'):
            try:
                ...
                logger.error("Response data: {response_data}")
            except Exception:
                logger.error("Could not parse response data from error")

        print("Failed to process sandbox {sandbox_id}: {error_type} - {str(e)}")
        return False
...
async def process_sandboxes(old_projects: List[Dict[str, Any]], dry_run: bool) -> tuple[int, int]:
    ...
    if dry_run:
        logger.info("DRY RUN: Would archive {len(old_projects)} sandboxes")
    else:
        logger.info("Archiving {len(old_projects)} sandboxes")

    print("Processing {len(old_projects)} sandboxes...")
    ...
    for i, project in enumerate(old_projects):
        ...
        if (i + 1) % 20 == 0 or (i + 1) == len(old_projects):
            ...
            print("Progress: {i + 1}/{len(old_projects)} sandboxes processed ({progress:.1f}%)")
            print("  - Processed: {processed_count}, Failed: {failed_count}")
...
async def main():
    ...
    logger.info("Starting sandbox cleanup for projects older than {args.days} day(s)")
    if args.dry_run:
    ...
    try:
        ...
        if not old_projects:
            logger.info("No projects older than {args.days} day(s) with sandboxes to process")
            print("No projects older than {args.days} day(s) with sandboxes to archive.")
            return

        # Print summary of what will be processed
        print("\n===== SANDBOX CLEANUP SUMMARY =====")
        print("Projects older than {args.days} day(s): {len(old_projects)}")
        print("Sandboxes that will be archived: {len(old_projects)}")
        print("===================================")

        logger.info("Found {len(old_projects)} projects older than {args.days} day(s)")
        ...
        for i, project in enumerate(old_projects[:5]):  # Just show first 5 for brevity
            ...
            print("{i+1}. Project: {project_name}")
            print("   Project ID: {project_id}")
            print("   Created At: {created_at}")
            print("   Sandbox ID: {sandbox_id}")

        if len(old_projects) > 5:
            print("   ... and {len(old_projects) - 5} more projects")
        ...
        # Print final summary
        print("\nSandbox Cleanup Summary:")
        print("Total projects older than {args.days} day(s): {len(old_projects)}")
        print("Total sandboxes processed: {len(old_projects)}")

        if args.dry_run:
            print("DRY RUN: No sandboxes were actually archived")
        else:
            print("Successfully processed: {processed_count}")
            print("Failed to process: {failed_count}")
