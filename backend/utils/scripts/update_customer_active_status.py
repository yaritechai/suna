...
async def check_stripe_subscription(customer_id: str) -> bool:
    ...
    """
    if not customer_id:
        print("⚠️ Empty customer_id")
        return False

    try:
        # Print what we're checking for debugging
        print("Checking Stripe subscriptions for customer: {customer_id}")
        ...
        print("Stripe returned data: {subscriptions.data}")

        # If there's at least one active subscription, the customer is active
        has_active_subscription = len(subscriptions.data) > 0

        if has_active_subscription:
            print("✅ Customer {customer_id} has ACTIVE subscription")
        else:
            print("❌ Customer {customer_id} has NO active subscription")

        return has_active_subscription

    except Exception as e:
        logger.error("Error checking Stripe subscription for customer {customer_id}: {str(e)}")
        print("⚠️ Error checking subscription for {customer_id}: {str(e)}")
        return False
...
async def process_customer_batch(batch: List[Dict[str, Any]], batch_number: int, total_batches: int) -> Dict[str, bool]:
    ...
    print("Batch {batch_number} completed in {end_time - start_time:.2f} seconds")
    print("Results (batch {batch_number}): {active_count} active, {inactive_count} inactive subscriptions")
...
async def update_customer_batch(subscription_status: Dict[str, bool]) -> Dict[str, int]:
    ...
    if active_customers:
        try:
            ...
            logger.info("Updated {len(active_customers)} customers to ACTIVE status")
        except Exception as e:
            logger.error("Error updating active customers: {str(e)}")
            stats['errors'] += 1
    ...
    if inactive_customers:
        try:
            ...
            logger.info("Updated {len(inactive_customers)} customers to INACTIVE status")
        except Exception as e:
            logger.error("Error updating inactive customers: {str(e)}")
            stats['errors'] += 1

    end_time = time.time()
    print("Database updates completed in {end_time - start_time:.2f} seconds")
...
async def main():
    ...
    try:
        ...
        print("Stripe API key configured: {'Yes' if config.STRIPE_SECRET_KEY else 'No'}")
        ...
        for i, customer in enumerate(all_customers[:5]):  # Show first 5 only
            print("  {i+1}. ID: {customer['id']}, Active: {customer.get('active')}")
        if len(all_customers) > 5:
            print("  ... and {len(all_customers) - 5} more")
        ...
        for i, batch in enumerate(batches):
            ...
            print("Completed batch {batch_number}/{total_batches}")
        ...
        print("Total customers processed: {all_stats['total']}")
        print("Customers set to active: {all_stats['active_updated']}")
        print("Customers set to inactive: {all_stats['inactive_updated']}")
        if all_stats['errors'] > 0:
            print("Update errors: {all_stats['errors']}")
        print("Total processing time: {total_time:.2f} seconds")

        logger.info("Customer active status update completed in {total_time:.2f} seconds")
