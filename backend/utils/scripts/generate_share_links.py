...
async def get_random_thread_ids(n: int) -> List[str]:
    ...
    while has_more:
        ...
        print("Fetching page {current_page + 1} (rows {start_range}-{end_range})...")
        ...
        try:
            ...
            if not result.data:
                ...
                print("Loaded {len(page_thread_ids)} thread IDs (total so far: {len(all_thread_ids)})")
    ...
    print("Found {len(all_thread_ids)} total thread IDs in database")
    ...
    if len(all_thread_ids) <= n:
        logger.warning("Requested {n} threads but only {len(all_thread_ids)} available. Returning all.")
        selected_thread_ids = all_thread_ids
    else:
        selected_thread_ids = random.sample(all_thread_ids, n)

    logger.info("Retrieved {len(selected_thread_ids)} random thread IDs")
...
async def generate_share_links(n: int) -> List[str]:
    try:
        ...
        share_links = ["suna.so/share/{thread_id}" for thread_id in thread_ids]

        logger.info("Generated {len(share_links)} share links")
        return share_links

    except Exception as e:
        logger.error("Error generating share links: {str(e)}")
        raise
...
def save_links_to_file(share_links: List[str], filename: str = None) -> str:
    ...
    if filename is None:
        ...
        filename = "share_links_{timestamp}.txt"

    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("Share Links Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 60 + "\n\n")
        ...
        logger.info("Share links saved to {filename}")
        return filename

    except Exception as e:
        logger.error("Error saving links to file: {str(e)}")
        raise
...
async def main():
    ...
    try:
        ...
        print("\nTotal: {len(share_links)} share links generated")
        print("Links saved to: {saved_filename}")
        logger.info("Share link generation completed")

    except Exception as e:
        logger.error("Error during share link generation: {str(e)}")
        sys.exit(1)
    finally:
        if db_connection:
