...
async def get_active_billing_customer_account_ids() -> Set[str]:
    ...
    print("Found {len(result.data)} billing customers in database")
    print(result.data)
    ...
    print("Found {len(active_account_ids)} active billing customers")
...
async def get_all_projects() -> List[Dict[str, Any]]:
    ...
    print("Found {total_projects} projects in database")
    ...
    logger.info("Found {len(projects_with_sandboxes)} projects with sandboxes")
...
async def archive_sandbox(project: Dict[str, Any], dry_run: bool) -> bool:
    ...
    try:
        logger.info("Checking sandbox {sandbox_id} for project '{project_name}' (ID: {project_id})")

        if dry_run:
            logger.info("DRY RUN: Would archive sandbox {sandbox_id}")
            print("Would archive sandbox {sandbox_id} for project '{project_name}'")
            return True
...
async def process_sandboxes(inactive_projects: List[Dict[str, Any]], dry_run: bool) -> tuple[int, int]:
    ...
    if dry_run:
        logger.info("DRY RUN: Would archive {len(inactive_projects)} sandboxes")
    else:
        logger.info("Archiving {len(inactive_projects)} sandboxes")

    print("Processing {len(inactive_projects)} sandboxes...")
    ...
    for i, project in enumerate(inactive_projects):
        ...
        if (i + 1) % 20 == 0 or (i + 1) == len(inactive_projects):
            ...
            print("Progress: {i + 1}/{len(inactive_projects)} sandboxes processed ({progress:.1f}%)")
...
async def main():
    ...
    try:
        ...
        print("Total projects found: {len(all_projects)}")
        print("Projects with active billing accounts: {active_projects_count}")
        print("Projects without active billing accounts: {len(inactive_projects)}")
        print("Sandboxes that will be archived: {len(inactive_projects)}")
        print("===================================")

        logger.info("Found {len(inactive_projects)} projects without an active billing customer account")
        ...
        if len(inactive_projects) > 5:
            print("   ... and {len(inactive_projects) - 5} more projects")
        ...
        # Print final summary
        print("\nSandbox Cleanup Summary:")
        print("Total projects without active billing: {len(inactive_projects)}")
        print("Total sandboxes processed: {len(inactive_projects)}")
