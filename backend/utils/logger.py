...
def setup_logger(name: str = 'agentpress') -> logging.Logger:
    ...
    try:
        log_file = os.path.join(log_dir, '{name}_{datetime.now().strftime("%Y%m%d")}.log')
        ...
        print("Added file handler for: {log_file}")
    except Exception as e:
        print("Error setting up file handler: {e}")
    ...
    try:
        ...
        logger.info("Added console handler with level: {console_handler.level}")
        logger.info("Log file will be created at: {log_dir}")
    except Exception as e:
        print("Error setting up console handler: {e}")
