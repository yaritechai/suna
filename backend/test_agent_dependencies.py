...
async def test_redis_connection():
    ...
    try:
        ...
        print(f"❌ FAILED: Redis connection error: {str(e)}")
        return False
...
async def test_supabase_connection():
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Database query successful (found {result.count} profiles)")
        ...
        print(f"❌ FAILED: Supabase connection error: {str(e)}")
        return False
...
async def test_daytona_connection():
    ...
    try:
        ...
        print(f"✅ Daytona API connection successful (found {len(sandboxes)} sandboxes)")
        
        # Test quota/limits
        print(f"✅ Daytona service accessible")
        
        print("\n✅ PASSED: Daytona service connection working")
        return True
        
    except Exception as e:
        print(f"❌ FAILED: Daytona connection error: {str(e)}")
        return False
...
async def test_llm_api_connection():
    ...
    try:
        ...
        if response and response.get('choices'):
            print(f"✅ LLM API call successful (model: {config.MODEL_TO_USE})")
        ...
        print(f"❌ FAILED: LLM API connection error: {str(e)}")
        return False
...
async def test_redis_broker_connection():
    ...
    try:
        ...
        print(f"✅ Redis broker initialized: {type(redis_broker).__name__}")
        ...
        if hasattr(redis_broker, 'client'):
            try:
                ...
                print(f"⚠️  Redis broker ping failed: {str(ping_error)}")
        
        print("\n✅ PASSED: Redis broker configured")
        return True
        
    except Exception as e:
        print(f"❌ FAILED: Redis broker error: {str(e)}")
        return False
...
async def test_model_resolution():
    ...
    try:
        ...
        print(f"✅ Default model configured: {config.MODEL_TO_USE}")
        
        # Test alias resolution
        resolved_model = MODEL_NAME_ALIASES.get(config.MODEL_TO_USE, config.MODEL_TO_USE)
        print(f"✅ Model resolves to: {resolved_model}")
        
        print("\n✅ PASSED: Model resolution working")
        return True
        
    except Exception as e:
        print(f"❌ FAILED: Model resolution error: {str(e)}")
        return False
...
async def main():
    ...
    if passed == total:
        print(f"✅ ALL TESTS PASSED ({passed}/{total}) - Agent initialization should work!")
    else:
        print(f"❌ TESTS FAILED ({passed}/{total}) - Agent initialization will fail!")
