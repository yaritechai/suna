...
async def test_custom_mcp():
    ...
    try:
        ...
        for tool in tools:
            print("   - {tool.get('name', 'Unknown')}: {tool.get('description', 'No description')}")

        # Get schemas
        schemas = wrapper.get_schemas()
        print("   ✅ Generated {len(schemas)} tool schemas")
        ...
        for method_name, schema_list in schemas.items():
            ...
            for schema in schema_list:
                if hasattr(schema, 'schema') and 'function' in schema.schema:
                    ...
                    print("     Function: {func_name} - {func_desc}")

        await wrapper.cleanup()
        print("   ✅ JSON Custom MCP test completed")

    except Exception as e:
        print("   ❌ JSON Custom MCP test failed: {e}")
    ...
    try:
        ...
        print("   ⚠️  SSE Custom MCP test failed: {e} (expected if endpoint not accessible)")
