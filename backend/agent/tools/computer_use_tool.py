...
class ComputerUseTool(SandboxToolsBase):
    ...
    def __init__(self, sandbox: Sandbox):
        ...
        logging.info("Initialized Computer Use Tool with API URL: {self.api_base_url}")
    ...
    async def _api_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            url = "{self.api_base_url}/api{endpoint}"

            logging.debug("API request: {method} {url} {data}")
            ...
            logging.debug("API response: {result}")
            return result
    ...
    async def move_to(self, x: float, y: float) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                self.mouse_x = x_int
                self.mouse_y = y_int
                return ToolResult(success=True, output="Moved to ({x_int}, {y_int})")
            else:
                return ToolResult(success=False, output="Failed to move: {result.get('error', 'Unknown error')}")
        ...
        try:
            ...
            if result.get("success", False):
                ...
                return ToolResult(success=True,
                                output="{num_clicks} {button} click(s) performed at ({x_int}, {y_int})")
            else:
                return ToolResult(success=False, output="Failed to click: {result.get('error', 'Unknown error')}")
        except Exception as e:
    ...
    async def scroll(self, amount: int) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                ...
                steps = abs(amount)
                return ToolResult(success=True,
                                output="Scrolled {direction} {steps} step(s) at position ({self.mouse_x}, {self.mouse_y})")
            else:
                return ToolResult(success=False, output="Failed to scroll: {result.get('error', 'Unknown error')}")
        except Exception as e:
    ...
    async def typing(self, text: str) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                ...
                return ToolResult(success=False, output="Failed to type: {result.get('error', 'Unknown error')}")
        except Exception as e:
    ...
    async def press(self, key: str) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                return ToolResult(success=True, output="Pressed key: {key}")
            else:
                return ToolResult(success=False, output="Failed to press key: {result.get('error', 'Unknown error')}")
        except Exception as e:
    ...
    async def wait(self, duration: float = 0.5) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=True, output="Waited {duration} seconds")
        except Exception as e:
    ...
    async def mouse_down(self, button: str = "left", x: Optional[float] = None, y: Optional[float] = None) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                self.mouse_x = x_int
                self.mouse_y = y_int
                return ToolResult(success=True, output="{button} button pressed at ({x_int}, {y_int})")
            else:
                return ToolResult(success=False, output="Failed to press button: {result.get('error', 'Unknown error')}")
        except Exception as e:
    ...
    async def mouse_up(self, button: str = "left", x: Optional[float] = None, y: Optional[float] = None) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                self.mouse_x = x_int
                self.mouse_y = y_int
                return ToolResult(success=True, output="{button} button released at ({x_int}, {y_int})")
            else:
                return ToolResult(success=False, output="Failed to release button: {result.get('error', 'Unknown error')}")
        except Exception as e:
    ...
    async def drag_to(self, x: float, y: float) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                ...
                return ToolResult(success=True,
                                output="Dragged from ({start_x}, {start_y}) to ({target_x}, {target_y})")
            else:
                return ToolResult(success=False, output="Failed to drag: {result.get('error', 'Unknown error')}")
        except Exception as e:
            return ToolResult(success=False, output="Failed to drag: {str(e)}")
    ...
    async def get_screenshot_base64(self) -> Optional[dict]:
        ...
        try:
            ...
            if "image" in result:
                ...
                timestamped_filename = os.path.join(screenshots_dir, "screenshot_{timestamp}.png")
    ...
    async def hotkey(self, keys: str) -> ToolResult:
        ...
        try:
            ...
            if result.get("success", False):
                return ToolResult(success=True, output="Pressed key combination: {keys}")
            else:
                return ToolResult(success=False, output="Failed to press keys: {result.get('error', 'Unknown error')}")
        except Exception as e:
            return ToolResult(success=False, output="Failed to press keys: {str(e)}")
