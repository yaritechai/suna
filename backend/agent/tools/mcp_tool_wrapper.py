...
class MCPToolWrapper(Tool):
            ...
            if standard_configs:
                for config in standard_configs:
                    try:
                        ...
                        logger.error("Failed to connect to MCP server {config['qualifiedName']}: {e}")
            ...
            try:
                async with sse_client(url, headers=headers) as (read, write):
                    async with ClientSession(read, write) as session:
                        ...
                        logger.info("  {server_name}: Connected via SSE ({len(tools_info)} tools)")
        ...
        async with asyncio.timeout(timeout):
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    logger.info("  {server_name}: Connected via stdio ({len(tools_info)} tools)")
    ...
    async def _initialize_custom_mcps(self, custom_configs):
        ...
        for config in custom_configs:
            try:
                ...
                logger.info("Initializing custom MCP: {server_name} (type: {custom_type})")

                if custom_type == 'sse':
                    if 'url' not in server_config:
                        logger.error("Custom MCP {server_name}: Missing 'url' in config")
                        continue
                        ...
                        if server_name in all_tools and all_tools[server_name].get('status') == 'connected':
                            ...
                            for tool_info in tools_info:
                                ...
                                if not enabled_tools or tool_name_from_server in enabled_tools:
                                    ...
                                    tools_registered += 1
                                    logger.debug("Registered custom tool: {tool_name}")

                            logger.info("Successfully initialized custom MCP {server_name} with {tools_registered} tools")
                        else:
                            logger.error("Failed to connect to custom MCP {server_name}")

                    except Exception as e:
                        logger.error("Custom MCP {server_name}: Connection failed - {str(e)}")
                        continue
                    ...
                    url = server_config['url']
                    logger.info("Initializing custom MCP {url} with HTTP type")
                    ...
                    if 'command' not in server_config:
                        logger.error("Custom MCP {server_name}: Missing 'command' in config")
                        continue
                    ...
                    logger.error("Custom MCP {server_name}: Unsupported type '{custom_type}', supported types are 'sse', 'http' and 'json'")
                    continue

            except Exception as e:
                logger.error("Failed to initialize custom MCP {config.get('name', 'Unknown')}: {e}")
                continue
    ...
    async def initialize_and_register_tools(self, tool_registry=None):
        ...
        if tool_registry and self._dynamic_tools:
            logger.info("Updating tool registry with {len(self._dynamic_tools)} MCP tools")
    ...
    async def _create_dynamic_tools(self):
        ...
        try:
            ...
            logger.info("Created {len(self._dynamic_tools)} dynamic MCP tool methods")

        except Exception as e:
            logger.error("Error creating dynamic MCP tools: {e}")
    ...
    def _create_dynamic_method(self, tool_name: str, tool_info: Dict[str, Any]):
        ...
        dynamic_tool_method.__qualname__ = "{self.__class__.__name__}.{method_name}"

        # Build a more descriptive description
        base_description = tool_info.get("description", "MCP tool from {server_name}")
        ...
        logger.debug("Created dynamic method '{method_name}' for MCP tool '{tool_name}' from server '{server_name}'")
    ...
    def _register_schemas(self):
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            if hasattr(method, 'tool_schemas'):
                ...
                logger.debug("Registered schemas for method '{name}' in {self.__class__.__name__}")

        # Note: Dynamic schemas will be added after async initialization
        logger.debug("Initial registration complete for MCPToolWrapper")
    ...
    async def _execute_mcp_tool(self, tool_name: str, arguments: Dict[str, Any]) -> ToolResult:
        ...
        await self._ensure_initialized()
        logger.info("Executing MCP tool {tool_name} with arguments {arguments}")
        try:
            ...
            logger.error("Error executing MCP tool {tool_name}: {str(e)}")
            return self.fail_response("Error executing tool: {str(e)}")
    ...
    async def _execute_custom_mcp_tool(self, tool_name: str, arguments: Dict[str, Any], tool_info: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if custom_type == 'sse':
                ...
                return self.fail_response("Unsupported custom MCP type: {custom_type}")

        except asyncio.TimeoutError:
            return self.fail_response("Tool execution timeout for {tool_name}")
        except Exception as e:
            logger.error("Error executing custom MCP tool {tool_name}: {str(e)}")
    ...
    async def cleanup(self):
        ...
        if self._initialized:
            try:
                ...
                logger.error("Error during MCP cleanup: {str(e)}")
            finally:
                self._initialized = False
