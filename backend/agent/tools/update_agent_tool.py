...
class UpdateAgentTool(Tool):
    ...
    async def get_current_agent_config(self) -> ToolResult:
        ...
        try:
            ...
            return self.success_response({
                "summary": "Agent '{config_summary['name']}' has {tools_count} tools enabled and {mcps_count} MCP servers configured.",
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.get(
                    "{self.smithery_api_base_url}/servers",
                    headers=headers,
                    params=params,
                    timeout=30.0
                )
    ...
    async def get_mcp_server_tools(self, qualified_name: str) -> ToolResult:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if self.smithery_api_key:
                    headers["Authorization"] = "Bearer {self.smithery_api_key}"
                ...
                url = "{self.smithery_api_base_url}/servers/{encoded_name}"
            ...
            try:
                    ...
                    for key, value in security.items():
                        if isinstance(value, dict):
                            config_requirements.append({
                                "name": key,
                                "description": value.get("description", "Configuration for {key}"),
                ...
                return self.success_response({
                    "message": "Found {len(formatted_tools)} tools for {server_info['name']}",
                    "server": server_info
                })
                ...
                return self.success_response({
                    "message": "Found {len(formatted_tools)} tools for {server_info['name']} (from registry)",
                    "server": server_info
                })
    ...
    async def configure_mcp_server(
        ...
        try:
            ...
            return self.success_response({
                "message": "Successfully {action} MCP server '{display_name}' with {len(enabled_tools)} tools",
    ...
    async def get_popular_mcp_servers(self, category: Optional[str] = None) -> ToolResult:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                return self.success_response({
                    "message": "Found popular MCP servers" + (" in category '{category}'" if category else ""),
                    "categorized_servers": categorized,
                    "total_categories": len(categorized)
                })

        except Exception as e:
            return self.fail_response("Error getting popular MCP servers: {str(e)}")
        ...
        try:
            ...
            async with streamablehttp_client(server_url) as (read_stream, write_stream, _):
                async with ClientSession(read_stream, write_stream) as session:
                    ...
                    return self.success_response({
                        "message": "Successfully connected to {qualified_name}",
            ...
            return self.fail_response("Failed to connect to {qualified_name}: {str(e)}")
