...
class SandboxWebSearchTool(SandboxToolsBase):
    ...
    async def web_search(
        ...
        try:
            ...
            logging.info("Executing web search for query: '{query}' with {num_results} results")
            ...
            logging.info("Retrieved search results for query: '{query}' with answer and {len(results)} results")
            ...
            if len(results) > 0 or (answer and answer.strip()):
                ...
                logging.warning("No search results or answer found for query: '{query}'")
                return ToolResult(
                    success=False,
                    output=json.dumps(search_response, ensure_ascii=False)
                )
            ...
            error_message = str(e)
            logging.error("Error performing web search for '{query}': {error_message}")
            simplified_message = "Error performing web search: {error_message[:200]}"
    ...
    async def scrape_webpage(
        ...
        try:
            ...
            logging.info("Starting to scrape webpages: {urls}")
            ...
            logging.info("Processing {len(url_list)} URLs: {url_list}")
            ...
            for url in url_list:
                try:
                    ...
                    if not (url.startswith('http://') or url.startswith('https://')):
                        url = 'https://' + url
                        logging.info("Added https:// protocol to URL: {url}")
            ...
            if successful == len(results):
                message = "Successfully scraped all {len(results)} URLs. Results saved to:"
                for r in results:
                    if r.get("file_path"):
                        message += "\n- {r.get('file_path')}"
            elif successful > 0:
                message = "Scraped {successful} URLs successfully and {failed} failed. Results saved to:"
                for r in results:
                ...
                error_details = "; ".join(["{r.get('url')}: {r.get('error', 'Unknown error')}" for r in results])
                return self.fail_response("Failed to scrape all {len(results)} URLs. Errors: {error_details}")
            ...
            error_message = str(e)
            logging.error("Error in scrape_webpage: {error_message}")
            return self.fail_response("Error processing scrape request: {error_message[:200]}")

    async def _scrape_single_url(self, url: str) -> dict:
        """
        Helper function to scrape a single URL and return the result information.
        """
        logging.info("Scraping single URL: {url}")
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                headers = {
                    "Authorization": "Bearer {self.firecrawl_api_key}",
                ...
                while retry_count < max_retries:
                    try:
                        logging.info("Sending request to Firecrawl (attempt {retry_count + 1}/{max_retries})")
                        response = await client.post(
                            "{self.firecrawl_url}/v1/scrape",
                            json=payload,
                            headers=headers,
                        ...
                        data = response.json()
                        logging.info("Successfully received response from Firecrawl for {url}")
                        break
                    except (httpx.ReadTimeout, httpx.ConnectTimeout, httpx.ReadError) as timeout_err:
                        retry_count += 1
                        logging.warning("Request timed out (attempt {retry_count}/{max_retries}): {str(timeout_err)}")
                        if retry_count >= max_retries:
                            raise Exception("Request timed out after {max_retries} attempts with {timeout_seconds}s timeout")
                        # Exponential backoff
                        logging.info("Waiting {2 ** retry_count}s before retry")
            ...
            if "metadata" in data.get("data", {}):
                ...
                logging.info("Added metadata: {data['data']['metadata'].keys()}")
            ...
            safe_filename = "{timestamp}_{domain}.json"

            logging.info("Generated filename: {safe_filename}")

            # Save results to a file in the /workspace/scrape directory
            scrape_dir = "{self.workspace_path}/scrape"
            self.sandbox.fs.create_folder(scrape_dir, "755")

            results_file_path = "{scrape_dir}/{safe_filename}"
