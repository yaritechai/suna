...
class DBConnection:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.debug("Database connection initialized with Supabase using {key_type}")
        except Exception as e:
            logger.error("Database initialization error: {e}")
            raise RuntimeError("Failed to initialize database connection: {str(e)}")
    ...
    async def upload_base64_image(self, base64_data: str, bucket_name: str = "browser-screenshots") -> str:
        ...
        try:
            ...
            logger.debug("Successfully uploaded image to {public_url}")
            return public_url

        except Exception as e:
            logger.error("Error uploading base64 image: {e}")
            raise RuntimeError("Failed to upload image: {str(e)}")
