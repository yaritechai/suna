...
async def transcribe_audio(
    ...
    try:
        ...
        logger.info("Received audio file: {audio_file.filename}, content_type: {audio_file.content_type}")

        if audio_file.content_type not in allowed_types:
            raise HTTPException(
                status_code=400,
                detail="Unsupported file type: {audio_file.content_type}. Supported types: {', '.join(allowed_types)}"
            )
        ...
        try:
            ...
            logger.info("Successfully transcribed audio for user {user_id}")
            ...
            try:
                ...
                logger.warning("Failed to delete temporary file {temp_file_path}: {e}")

    except Exception as e:
        logger.error("Error transcribing audio for user {user_id}: {str(e)}")
