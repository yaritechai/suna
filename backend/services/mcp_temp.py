...
#     logger.info("Connecting to {server_name} using Windows subprocess method")
...
#         logger.info("  {server_name}: Connected via Windows subprocess ({len(result['tools'])} tools)")
#     else:
...
#         logger.info("Connecting to MCP server: {server_name}")
...
#                 "tools": []
#             }
#             logger.error("  {server_name}: Error - {error_msg}")
#             import traceback
#             logger.debug("Full traceback for {server_name}: {traceback.format_exc()}")

#     return all_tools


async def discover_custom_tools(request_type: str, config: Dict[str, Any]):
    logger.info("Received custom MCP discovery request: type={request_type}")
    ...
    #                     logger.info("Found {len(tools)} tools for server {server_name}")
    #                 else:
    #                     error_msg = server_info.get("error", "Unknown error")
    #                     logger.error("Server {server_name} failed: {error_msg}")
    #                     raise HTTPException(
    #                         status_code=400,
    #                         detail="Failed to connect to MCP server '{server_name}': {error_msg}"
    #                     )
    #             else:
    #                 logger.error("Server {server_name} not found in results")
    #                 raise HTTPException(status_code=400, detail="Server '{server_name}' not found in results")
    #         else:
    ...
    #         raise
    #     except Exception as e:
    #         logger.error("Error connecting to stdio MCP server: {e}")
    #         import traceback
    #         logger.error("Full traceback: {traceback.format_exc()}")
    #         raise HTTPException(status_code=400, detail="Failed to connect to MCP server: {str(e)}")
    ...
    #     except Exception as e:
    #         logger.error("Error connecting to SSE MCP server: {e}")
    #         raise HTTPException(status_code=400, detail="Failed to connect to MCP server: {str(e)}")
    # else:
    ...
    # logger.info("Returning {len(tools)} tools for server {server_name}")
