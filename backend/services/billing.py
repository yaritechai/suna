...
async def get_user_subscription(user_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        if len(our_subscriptions) > 1:
            logger.warning("User {user_id} has multiple active subscriptions: {[sub['id'] for sub in our_subscriptions]}")
            ...
            for sub in our_subscriptions:
                if sub['id'] != most_recent['id']:
                    try:
                        ...
                        logger.info("Cancelled subscription {sub['id']} for user {user_id}")
                    except Exception as e:
                        logger.error("Error cancelling subscription {sub['id']}: {str(e)}")
        ...
        logger.error("Error getting subscription from Stripe: {str(e)}")
        return None
...
async def can_use_model(client, user_id: str, model_name: str):
    ...
    return False, "Your current subscription plan does not include access to {model_name}. Please upgrade your subscription or choose from your available models: {', '.join(allowed_models)}", allowed_models
...
async def check_billing_status(client, user_id: str) -> Tuple[bool, str, Optional[Dict]]:
    ...
    tier_info = SUBSCRIPTION_TIERS.get(price_id)
    if not tier_info:
        logger.warning("Unknown subscription tier: {price_id}, defaulting to free tier")
    ...
    if current_usage >= tier_info['minutes']:
        return False, "Monthly limit of {tier_info['minutes']} minutes reached. Please upgrade your plan or wait until next month.", subscription
...
async def create_checkout_session(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid price ID: {request.price_id}")
                    ...
                    try:
                        ...
                        for item in current_phase.get('items', []):
                            ...
                            if price_id and quantity is not None:
                                ...
                                logger.warning("Skipping item in current phase due to missing price ID or quantity: {item}")
                        ...
                        if schedule_id:
                            ...
                            # print("Updating existing schedule {schedule_id}")
                            logger.info("Updating existing schedule {schedule_id} for subscription {subscription_id}")
                            logger.debug("Current phase data: {current_phase_update_data}")
                            logger.debug("New phase data: {new_downgrade_phase_data}")
                            updated_schedule = stripe.SubscriptionSchedule.modify(
                                schedule_id,
                                phases=[current_phase_update_data, new_downgrade_phase_data],
                                end_behavior='release'
                            )
                            logger.info("Successfully updated schedule {updated_schedule['id']}")
                        else:
                             # Create a new schedule using the defined phases
                            print("Creating new schedule for subscription {subscription_id}")
                            logger.info("Creating new schedule for subscription {subscription_id}")
                            # Deep debug logging - write subscription details to help diagnose issues
                            logger.debug("Subscription details: {subscription_id}, current_period_end_ts: {current_period_end_ts}")
                            ...
                            try:
                                ...
                                logger.info("Created new schedule {updated_schedule['id']} from subscription {subscription_id}")
                                # print("Created new schedule {updated_schedule['id']} from subscription {subscription_id}")

                                # Verify the schedule was created correctly
                                fetched_schedule = stripe.SubscriptionSchedule.retrieve(updated_schedule['id'])
                                logger.info("Schedule verification - Status: {fetched_schedule.get('status')}, Phase Count: {len(fetched_schedule.get('phases', []))}")
                                logger.debug("Schedule details: {fetched_schedule}")
                        ...
                        return {
                         ...
                         logger.exception("Error handling subscription schedule for sub {subscription_id}: {str(e)}")
                         raise HTTPException(status_code=500, detail="Error handling subscription schedule: {str(e)}")
            except Exception as e:
                logger.exception("Error updating subscription {existing_subscription.get('id') if existing_subscription else 'N/A'}: {str(e)}")
        ...
        if hasattr(e, 'json_body') and e.json_body and 'error' in e.json_body:
            ...
            error_detail = str(e)
        raise HTTPException(status_code=500, detail="Error creating checkout session: {error_detail}")
...
async def create_portal_session(
    ...
    try:
        ...
        try:
            ...
            for config in configurations.get('data', []):
                ...
                if subscription_update.get('enabled', False):
                    ...
                    logger.info("Found existing portal configuration with subscription_update enabled: {config['id']}")
                    break
            ...
            logger.info("Using portal configuration: {active_config['id']} with subscription_update: {active_config.get('features', {}).get('subscription_update', {}).get('enabled', False)}")
        ...
        logger.error("Error creating portal session: {str(e)}")
    ...
    try:
        ...
        if schedule_id:
            try:
                ...
                logger.error("Error retrieving or parsing schedule {schedule_id} for sub {subscription['id']}: {schedule_error}")
                # Proceed without schedule info if retrieval fails

        return status_response

    except Exception as e:
        logger.exception("Error getting subscription status for user {current_user_id}: {str(e)}") # Use logger.exception
...
async def check_status(
    ...
    try:
        ...
        logger.error("Error checking billing status: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
        ...
        if event.type in ['customer.subscription.created', 'customer.subscription.updated', 'customer.subscription.deleted']:
            ...
            if event.type == 'customer.subscription.created' or event.type == 'customer.subscription.updated':
                ...
                if not has_active:
                    ...
                    logger.info("Webhook: Updated customer {customer_id} active status to FALSE after subscription deletion")
...
async def get_available_models(
    ...
    try:
        ...
        logger.error("Error getting available models: {str(e)}")
        raise HTTPException(status_code=500, detail="Error getting available models: {str(e)}")
