...
class DOMElementNode(DOMBaseNode):
    ...
    def __repr__(self) -> str:
        ...
        if self.highlight_index is not None:
            extras.append('highlight:{self.highlight_index}')
        ...
        def process_node(node: DOMBaseNode, depth: int) -> None:
            if isinstance(node, DOMElementNode):
                ...
                if node.highlight_index is not None:
                    ...
                    line = '[{node.highlight_index}]<{node.tag_name}'

                    # Add important attributes for identification
                    for attr_name in ['id', 'hre', 'name', 'value', 'type']:
                        if attr_name in node.attributes and node.attributes[attr_name]:
                            line += ' {attr_name}="{node.attributes[attr_name]}"'
...
class BrowserAutomation:
    def __init__(self):
        ...
        self.include_attributes = ["id", "hre", "src", "alt", "aria-label", "placeholder", "name", "role", "title", "value"]
        ...
        self.router.post("/automation/save_pd")(self.save_pdf)
    ...
    async def startup(self):
        ...
        try:
            ...
            try:
                ...
                print("Failed to launch browser: {browser_error}")
            ...
            print("Browser startup error: {str(e)}")
            traceback.print_exc()
            raise RuntimeError("Browser initialization failed: {str(e)}")
    ...
    async def get_selector_map(self) -> Dict[int, DOMElementNode]:
        ...
        try:
            ...
            print("Found {len(elements)} interactive elements in selector map")
            ...
            print("Error getting selector map: {e}")
            traceback.print_exc()
    ...
    async def get_current_dom_state(self) -> DOMState:
        ...
        try:
            ...
            print("Error getting DOM state: {e}")
            traceback.print_exc()
    ...
    async def take_screenshot(self) -> str:
        ...
        try:
            ...
            try:
                ...
                print("Warning: Network idle timeout, proceeding anyway: {e}")
    ...
    async def save_screenshot_to_file(self) -> str:
        ...
        try:
            ...
            filename = "screenshot_{timestamp}_{random_id}.jpg"
    ...
    async def get_updated_browser_state(self, action_name: str) -> tuple:
        ...
        try:
            ...
            for idx, element in dom_state.selector_map.items():
                ...
                # Add key attributes
                for attr_name in ['id', 'hre', 'src', 'alt', 'placeholder', 'name', 'role', 'title', 'type']:
            ...
            try:
                ...
                print("Error getting viewport dimensions: {e}")
            ...
            print("Got updated state after {action_name}: {len(dom_state.selector_map)} elements")
            return dom_state, screenshot, elements, metadata
        except Exception as e:
            print("Error getting updated state after {action_name}: {e}")
            traceback.print_exc()
    ...
    async def navigate_to(self, action: GoToUrlAction = Body(...)):
        ...
        try:
            ...
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("navigate_to({action.url})")
            ...
            print("Navigation result: success={result.success}, url={result.url}")
            return result
        except Exception as e:
    ...
    async def search_google(self, action: SearchGoogleAction = Body(...)):
        ...
        try:
            page = await self.get_current_page()
            search_url = "https://www.google.com/search?q={action.query}"
            await page.goto(search_url)
            await page.wait_for_load_state()

            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("search_google({action.query})")

            return self.build_action_result(
                True,
                "Searched for '{action.query}' in Google",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def wait(self, seconds: int = Body(3)):
        ...
        try:
            ...
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("wait({seconds} seconds)")
    ...
    async def click_coordinates(self, action: ClickCoordinatesAction = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("click_coordinates({action.x}, {action.y})")

            return self.build_action_result(
                True,
                "Clicked at coordinates ({action.x}, {action.y})",
                dom_state,
                screenshot,
                elements,
                metadata,
            ...
            print("Error in click_coordinates: {e}")
            traceback.print_exc()
    ...
    async def click_element(self, action: ClickElementAction = Body(...)):
        ...
        try:
            ...
            if action.index not in selector_map:
                ...
                dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("click_element_error (index {action.index} not found)")
                return self.build_action_result(
                    False,
                    "Element with index {action.index} not found",
            ...
            print("Attempting to click element: {element_to_click}")
            ...
            if await target_element_handle.evaluate("node => node !== null"):
                try:
                    ...
                    click_success = True
                    print("Successfully clicked element handle for index {action.index}")
                except Exception as click_error:
                    error_message = "Error clicking element handle: {click_error}"
                    print(error_message)
                    # Optional: Add fallback methods here if needed
                    # e.g., target_element_handle.dispatch_event('click')
            else:
                 error_message = "Could not locate the target element handle for index {action.index} using JS script."
                 print(error_message)
            ...
            try:
                ...
                print("Timeout or error waiting for network idle after click: {wait_error}")
                await asyncio.sleep(1) # Fallback wait

            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("click_element({action.index})")

            return self.build_action_result(
                click_success,
                "Clicked element with index {action.index}" if click_success else "Attempted to click element {action.index} but failed. Error: {error_message}",
                dom_state,
                screenshot,
                elements,
                metadata,
            ...
            print("Error in click_element: {e}")
            traceback.print_exc()
            # Try to get state even after error
            try:
    ...
    async def input_text(self, action: InputTextAction = Body(...)):
        ...
        try:
            ...
            if element.attributes.get("id"):
                await page.fill("#{element.attributes['id']}", action.text)
            elif element.attributes.get("class"):
                class_selector = ".{element.attributes['class'].replace(' ', '.')}"
                await page.fill(class_selector, action.text)
            else:
                # Fallback to xpath
                await page.fill("//{element.tag_name}[{action.index}]", action.text)

            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("input_text({action.index}, '{action.text}')")

            return self.build_action_result(
                True,
                "Input '{action.text}' into element with index {action.index}",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def send_keys(self, action: SendKeysAction = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("send_keys({action.keys})")
    ...
    async def switch_tab(self, action: SwitchTabAction = Body(...)):
        ...
        try:
            if 0 <= action.page_id < len(self.pages):
                ...
                # Get updated state after action
                dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("switch_tab({action.page_id})")

                return self.build_action_result(
                    True,
                    "Switched to tab {action.page_id}",
                    dom_state,
                    screenshot,
                    elements,
                    metadata,
                ...
                return self.build_action_result(
                    False,
                    "Tab {action.page_id} not found",
                    None,
                    "",
                    "",
                    {},
                    error="Tab {action.page_id} not found"
                )
        except Exception as e:
            return self.build_action_result(
                False,
    ...
    async def open_tab(self, action: OpenTabAction = Body(...)):
        ...
        try:
            print("Attempting to open new tab with URL: {action.url}")
            ...
            print("Navigated to URL in new tab: {action.url}")

            # Add to page list and make it current
            self.pages.append(new_page)
            self.current_page_index = len(self.pages) - 1
            print("New tab added as index {self.current_page_index}")

            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("open_tab({action.url})")

            return self.build_action_result(
                True,
                "Opened new tab with URL: {action.url}",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def close_tab(self, action: CloseTabAction = Body(...)):
        ...
        try:
            if 0 <= action.page_id < len(self.pages):
                ...
                # Get updated state after action
                page = await self.get_current_page()
                dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("close_tab({action.page_id})")

                return self.build_action_result(
                    True,
                    "Closed tab {action.page_id} with URL: {url}",
                    dom_state,
                    screenshot,
                    elements,
                    metadata,
    ...
    async def extract_content(self, goal: str = Body(...)):
        ...
        try:
            ...
            # Get updated state
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("extract_content({goal})")

            return self.build_action_result(
                True,
                "Content extracted based on goal: {goal}",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def save_pdf(self):
        ...
        try:
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            random_id = random.randint(1000, 9999)
            filename = "page_{timestamp}_{random_id}.pdf"
            filepath = os.path.join(self.screenshot_dir, filename)
    ...
    async def scroll_down(self, action: ScrollAction = Body(...)):
        ...
        try:
            ...
            if action.amount is not None:
                await page.evaluate("window.scrollBy(0, {action.amount});")
                amount_str = "{action.amount} pixels"
            else:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("scroll_down({amount_str})")

            return self.build_action_result(
                True,
                "Scrolled down by {amount_str}",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def scroll_up(self, action: ScrollAction = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("scroll_up({amount_str})")

            return self.build_action_result(
                True,
                "Scrolled up by {amount_str}",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def scroll_to_text(self, text: str = Body(...)):
        ...
        try:
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("scroll_to_text({text})")

            message = "Scrolled to text: {text}" if found else "Text '{text}' not found or not visible on page"
        ...
        try:
            ...
            try:
                if element.tag_name.lower() == 'select':
                    ...
                    await page.click("#{element.attributes.get('id')}") if element.attributes.get('id') else None
            ...
            # Get updated state
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("get_dropdown_options({index})")

            return self.build_action_result(
                True,
                "Retrieved {len(options)} options from dropdown",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def select_dropdown_option(self, index: int = Body(...), option_text: str = Body(...)):
        ...
        try:
            ...
            if element.tag_name.lower() == 'select':
                ...
                selector = "select option:has-text('{option_text}')"
                await page.select_option(
                    "#{element.attributes.get('id')}" if element.attributes.get('id') else "//select[{index}]",
                    label=option_text
                )
            else:
                ...
                if element.attributes.get('id'):
                    await page.click("#{element.attributes.get('id')}")
                else:
                    await page.click("//{element.tag_name}[{index}]")

                await page.wait_for_timeout(500)

                # Then try to click the option
                await page.click("text={option_text}")

            await page.wait_for_timeout(500)

            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("select_dropdown_option({index}, '{option_text}')")

            return self.build_action_result(
                True,
                "Selected option '{option_text}' from dropdown with index {index}",
                dom_state,
                screenshot,
                elements,
                metadata,
    ...
    async def drag_drop(self, action: DragDropAction = Body(...)):
        ...
        try:
            ...
            if action.element_source and action.element_target:
                ...
                message = "Dragged element '{source_desc}' to '{target_desc}'"
            ...
            # Coordinate-based drag and drop
                ...
                message = "Dragged from ({source_x}, {source_y}) to ({target_x}, {target_y})"
            else:
                return self.build_action_result(
                    False,
            ...
            # Get updated state after action
            dom_state, screenshot, elements, metadata = await self.get_updated_browser_state("drag_drop({action.element_source}, {action.element_target})")
...
async def test_browser_api():
    ...
    try:
        ...
        print("Navigation status: {'✅ Success' if result.success else '❌ Failed'}")
        ...
        if result.interactive_elements and len(result.interactive_elements) > 0:
            ...
            for el in result.interactive_elements:
                print("  [{el['index']}] <{el['tag_name']}> {el.get('text', '')[:30]}")

        # Screenshot info
        print("\nScreenshot captured: {'Yes' if result.screenshot_base64 else 'No'}")
        print("Viewport size: {result.viewport_width}x{result.viewport_height}")
        ...
        if result.ocr_text:
            ...
            print("=== OCR TEXT END ===")
            print("OCR text length: {len(result.ocr_text)} characters")
            print(result.ocr_text)
        else:
        ...
        print("Search status: {'✅ Success' if result.success else '❌ Failed'}")
        if not result.success:
            print("Error: {result.error}")
        else:
            print("Found {result.element_count} elements after search")
        ...
        print("Scroll status: {'✅ Success' if result.success else '❌ Failed'}")
        if result.success:
            print("Pixels above viewport: {result.pixels_above}")
            print("Pixels below viewport: {result.pixels_below}")
        ...
        if result.element_count > 0:
            ...
            print("Click status: {'✅ Success' if click_result.success else '❌ Failed'}")
            print("Message: {click_result.message}")
            print("New URL after click: {click_result.url}")
        else:
        ...
        print("Coordinate click status: {'✅ Success' if coord_click_result.success else '❌ Failed'}")
        print("Message: {coord_click_result.message}")
        print("URL after coordinate click: {coord_click_result.url}")
        ...
        print("Content extraction status: {'✅ Success' if content_result.success else '❌ Failed'}")
        if content_result.content:
            ...
            print("Content sample: {content_preview}")
            print("Total content length: {len(content_result.content)} chars")
        else:
        ...
        print("New tab status: {'✅ Success' if tab_result.success else '❌ Failed'}")
        if tab_result.success:
            print("New tab title: {tab_result.title}")
            print("Interactive elements: {tab_result.element_count}")
...
async def test_browser_api_2():
    ...
    try:
        ...
        if result.element_count > 4: # Ensure element 5 exists
            ...
            print("Click status for element {click_index}: {'✅ Success' if click_result.success else '❌ Failed'}")
            print("Message: {click_result.message}")
            print("URL after click: {click_result.url}")
            ...
            if click_result.element_count > 0: # Check if there are still elements
                ...
                print("Click status for element {click_index_2}: {'✅ Success' if click_result_2.success else '❌ Failed'}")
                print("Message: {click_result_2.message}")
                print("URL after click: {click_result_2.url}")
