...
class XMLTagSchema:
    ...
    def add_mapping(self, param_name: str, node_type: str = "element", path: str = ".", required: bool = True) -> None:
        ...
        logger.debug("Added XML mapping for parameter '{param_name}' with type '{node_type}' at path '{path}', required={required}")
...
class Tool(ABC):
    ...
    def __init__(self):
        ...
        logger.debug("Initializing tool class: {self.__class__.__name__}")
    ...
    def success_response(self, data: Union[Dict[str, Any], str]) -> ToolResult:
        ...
        logger.debug("Created success response for {self.__class__.__name__}")
    ...
    def fail_response(self, msg: str) -> ToolResult:
        ...
        """
        logger.debug("Tool {self.__class__.__name__} returned failed result: {msg}")
...
def _add_schema(func, schema: ToolSchema):
    ...
    logger.debug("Added {schema.schema_type.value} schema to function {func.__name__}")
    return func

def openapi_schema(schema: Dict[str, Any]):
    """Decorator for OpenAPI schema tools."""
    def decorator(func):
        logger.debug("Applying OpenAPI schema to function {func.__name__}")
...
def xml_schema(
    ...
    """
    def decorator(func):
        logger.debug("Applying XML schema with tag '{tag_name}' to function {func.__name__}")
...
def custom_schema(schema: Dict[str, Any]):
    ...
    def decorator(func):
        logger.debug("Applying custom schema to function {func.__name__}")
        return _add_schema(func, ToolSchema(
