...
class ResponseProcessor:
    ...
    async def process_streaming_response(
        ...
        logger.info("Streaming Config: XML={config.xml_tool_calling}, Native={config.native_tool_calling}, "
        ...
        try:
            ...
            async for chunk in llm_response:
                ...
                if hasattr(chunk, 'choices') and chunk.choices and hasattr(chunk.choices[0], 'finish_reason') and chunk.choices[0].finish_reason:
                    ...
                    logger.debug("Detected finish_reason: {finish_reason}")
                ...
                if hasattr(chunk, 'choices') and chunk.choices:
                    ...
                    if delta and hasattr(delta, 'content') and delta.content:
                        ...
                        if not (config.max_xml_tool_calls > 0 and xml_tool_call_count >= config.max_xml_tool_calls):
                            ...
                            if self.trace:
                                self.trace.event(name="xml_tool_call_limit_reached", level="DEFAULT", status_message=("XML tool call limit reached - not yielding more content chunks"))
                            ...
                            for xml_chunk in xml_chunks:
                                ...
                                if result:
                                    ...
                                    if config.max_xml_tool_calls > 0 and xml_tool_call_count >= config.max_xml_tool_calls:
                                        logger.debug("Reached XML tool call limit ({config.max_xml_tool_calls})")
                ...
                if self.trace:
                    self.trace.event(name="stopping_stream_processing_after_loop_due_to_xml_tool_call_limit", level="DEFAULT", status_message=("Stopping stream processing after loop due to XML tool call limit"))
                    break
            ...
            if (
                ...
                logger.info(
                    "🔥 Estimated tokens – prompt: {prompt_tokens}, "
                    "completion: {completion_tokens}, total: {prompt_tokens + completion_tokens}"
                )
            ...
            if pending_tool_executions:
                logger.info("Waiting for {len(pending_tool_executions)} pending streamed tool executions")
                if self.trace:
                    self.trace.event(name="waiting_for_pending_streamed_tool_executions", level="DEFAULT", status_message=("Waiting for {len(pending_tool_executions)} pending streamed tool executions"))
                # ... (asyncio.wait logic) ...
                ...
                for execution in pending_tool_executions:
                    ...
                    if tool_idx in yielded_tool_indices:
                         logger.debug("Status for tool index {tool_idx} already yielded.")
                         ...
                         try:
                             if execution["task"].done():
                                 ...
                                 if tool_name in ['ask', 'complete']:
                                     logger.info("Terminating tool '{tool_name}' completed during streaming. Setting termination flag.")
                                     if self.trace:
                                         self.trace.event(name="terminating_tool_completed_during_streaming", level="DEFAULT", status_message=("Terminating tool '{tool_name}' completed during streaming. Setting termination flag."))
                                     agent_should_terminate = True

                             else: # Should not happen with asyncio.wait
                                logger.warning("Task for tool index {tool_idx} not done after wait.")
                                if self.trace:
                                    self.trace.event(name="task_for_tool_index_not_done_after_wait", level="WARNING", status_message=("Task for tool index {tool_idx} not done after wait."))
                         except Exception as e:
                             logger.error("Error getting result for pending tool execution {tool_idx}: {str(e)}")
                             if self.trace:
                                 self.trace.event(name="error_getting_result_for_pending_tool_execution", level="ERROR", status_message=("Error getting result for pending tool execution {tool_idx}: {str(e)}"))
                             context.error = e
                    ...
                    try:
                        ...
                        logger.error("Error getting result/yielding status for pending tool execution {tool_idx}: {str(e)}")
                        if self.trace:
                            self.trace.event(name="error_getting_result_yielding_status_for_pending_tool_execution", level="ERROR", status_message=("Error getting result/yielding status for pending tool execution {tool_idx}: {str(e)}"))
                        context.error = e
            ...
            if finish_reason == "xml_tool_limit_reached":
                ...
                logger.info("Stream finished with reason: xml_tool_limit_reached after {xml_tool_call_count} XML tool calls")
                if self.trace:
            ...
            if accumulated_content:
                ...
                if last_assistant_message_object:
                    ...
                    logger.error("Failed to save final assistant message for thread {thread_id}")
                    if self.trace:
            ...
            if config.execute_tools:
                ...
                if config.execute_on_stream and tool_results_buffer:
                    logger.info("Processing {len(tool_results_buffer)} buffered tool results")
                    if self.trace:
                        self.trace.event(name="processing_buffered_tool_results", level="DEFAULT", status_message=("Processing {len(tool_results_buffer)} buffered tool results"))
                ...
                # Or execute now if not streamed
                    ...
                    logger.info("Executing {len(final_tool_calls_to_process)} tools ({config.tool_execution_strategy}) after stream")
                    if self.trace:
                        self.trace.event(name="executing_tools_after_stream", level="DEFAULT", status_message=("Executing {len(final_tool_calls_to_process)} tools ({config.tool_execution_strategy}) after stream"))
                    ...
                    for tc, res in results_list:
                       ...
                       if current_tool_idx in all_tool_data_map:
                           ...
                           logger.warning("Could not map result for tool index {current_tool_idx}")
                           if self.trace:
                               self.trace.event(name="could_not_map_result_for_tool_index", level="WARNING", status_message=("Could not map result for tool index {current_tool_idx}"))
                       current_tool_idx += 1

                # Save and Yield each result message
                if tool_results_map:
                    logger.info("Saving and yielding {len(tool_results_map)} final tool result messages")
                    if self.trace:
                        self.trace.event(name="saving_and_yielding_final_tool_result_messages", level="DEFAULT", status_message=("Saving and yielding {len(tool_results_map)} final tool result messages"))
                    for tool_idx in sorted(tool_results_map.keys()):
                        ...
                        if saved_tool_result_object:
                            ...
                            yield format_for_yield(saved_tool_result_object)
                             ...
                             logger.error("Failed to save tool result for index {tool_idx}, not yielding result message.")
                             if self.trace:
                                 self.trace.event(name="failed_to_save_tool_result_for_index", level="ERROR", status_message=("Failed to save tool result for index {tool_idx}, not yielding result message."))
            ...
            if agent_should_terminate:
                ...
                if last_assistant_message_object:
                    try:
                        ...
                        logger.error("Error saving assistant response end for stream (before termination): {str(e)}")
                        if self.trace:
                            self.trace.event(name="error_saving_assistant_response_end_for_stream_before_termination", level="ERROR", status_message=("Error saving assistant response end for stream (before termination): {str(e)}"))
            ...
            if last_assistant_message_object: # Only save if assistant message was saved
                try:
                    ...
                    logger.error("Error saving assistant response end for stream: {str(e)}")
                    if self.trace:
                        self.trace.event(name="error_saving_assistant_response_end_for_stream", level="ERROR", status_message=("Error saving assistant response end for stream: {str(e)}"))

        except Exception as e:
            logger.error("Error processing stream: {str(e)}", exc_info=True)
            if self.trace:
            ...
            logger.critical("Re-raising error to stop further processing: {str(e)}")
            if self.trace:
            ...
            try:
                ...
                logger.error("Error in finally block: {str(final_e)}", exc_info=True)
                if self.trace:
                    self.trace.event(name="error_in_finally_block", level="ERROR", status_message=("Error in finally block: {str(final_e)}"))
    ...
    async def process_non_streaming_response(
        ...
        try:
            ...
            if hasattr(llm_response, 'choices') and llm_response.choices:
                 if hasattr(llm_response.choices[0], 'finish_reason'):
                     ...
                     logger.info("Non-streaming finish_reason: {finish_reason}")
                     if self.trace:
                         self.trace.event(name="non_streaming_finish_reason", level="DEFAULT", status_message=("Non-streaming finish_reason: {finish_reason}"))
            ...
            if assistant_message_object:
                 ...
                 logger.error("Failed to save non-streaming assistant message for thread {thread_id}")
                 if self.trace:
       ...
       # --- Execute Tools and Yield Results ---
            ...
            if config.execute_tools and tool_calls_to_execute:
                logger.info("Executing {len(tool_calls_to_execute)} tools with strategy: {config.tool_execution_strategy}")
                if self.trace:
                    self.trace.event(name="executing_tools_with_strategy", level="DEFAULT", status_message=("Executing {len(tool_calls_to_execute)} tools with strategy: {config.tool_execution_strategy}"))
                ...
                for i, (returned_tool_call, result) in enumerate(tool_results):
                    ...
                    if saved_tool_result_object:
                        ...
                        yield format_for_yield(saved_tool_result_object)
                         ...
                         logger.error("Failed to save tool result for index {tool_index}")
                         if self.trace:
            ...
            if assistant_message_object: # Only save if assistant message was saved
                try:
                    ...
                    logger.error("Error saving assistant response end for non-stream: {str(e)}")
                    if self.trace:
                        self.trace.event(name="error_saving_assistant_response_end_for_non_stream", level="ERROR", status_message=("Error saving assistant response end for non-stream: {str(e)}"))

        except Exception as e:
             logger.error("Error processing non-streaming response: {str(e)}", exc_info=True)
             if self.trace:
    ...
    def _extract_tag_content(self, xml_chunk: str, tag_name: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            logger.error("Error extracting tag content: {e}")
            if self.trace:
                self.trace.event(name="error_extracting_tag_content", level="ERROR", status_message=("Error extracting tag content: {e}"))
            return None, xml_chunk
    ...
    def _extract_attribute(self, opening_tag: str, attr_name: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error("Error extracting attribute: {e}")
            if self.trace:
                self.trace.event(name="error_extracting_attribute", level="ERROR", status_message=("Error extracting attribute: {e}"))
            return None
    ...
    def _extract_xml_chunks(self, content: str) -> List[str]:
        ...
        try:
            ...
            logger.error("Error extracting XML chunks: {e}")
    ...
    def _parse_xml_tool_call(self, xml_chunk: str) -> Optional[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        try:
            ...
            if '<function_calls>' in xml_chunk and '<invoke' in xml_chunk:
                ...
                logger.debug("Parsed new format tool call: {tool_call}")
            ...
            if not tag_match:
                logger.error("No tag found in XML chunk: {xml_chunk}")
                if self.trace:
                    self.trace.event(name="no_tag_found_in_xml_chunk", level="ERROR", status_message=("No tag found in XML chunk: {xml_chunk}"))
                return None

            # This is the XML tag as it appears in the text (e.g., "create-file")
            xml_tag_name = tag_match.group(1)
            logger.info("Found XML tag: {xml_tag_name}")
            if self.trace:
                self.trace.event(name="found_xml_tag", level="DEFAULT", status_message=("Found XML tag: {xml_tag_name}"))
            ...
            if not tool_info or not tool_info['schema'].xml_schema:
                logger.error("No tool or schema found for tag: {xml_tag_name}")
                if self.trace:
                    self.trace.event(name="no_tool_or_schema_found_for_tag", level="ERROR", status_message=("No tool or schema found for tag: {xml_tag_name}"))
                return None
                ...
                try:
                    if mapping.node_type == "attribute":
                        ...
                        if value is not None:
                            ...
                            # logger.info("Found attribute {mapping.param_name}: {value}")
                        ...
                        if content is not None:
                            ...
                            # logger.info("Found element {mapping.param_name}: {content.strip()}")
                        ...
                        if content is not None:
                            ...
                            # logger.info("Found text content for {mapping.param_name}: {content.strip()}")
                        ...
                        if content is not None:
                            ...
                            # logger.info("Found root content for {mapping.param_name}")
            ...
            logger.debug("Parsed old format tool call: {tool_call}")
            return tool_call, parsing_details # Return both dicts

        except Exception as e:
            logger.error("Error parsing XML chunk: {e}")
            logger.error("XML chunk was: {xml_chunk}")
            if self.trace:
    ...
    def _parse_xml_tool_calls(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Error parsing XML tool calls: {e}", exc_info=True)
            if self.trace:
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> ToolResult:
        ...
        span = self.trace.span(name="execute_tool.{tool_call['function_name']}", input=tool_call["arguments"]) if self.trace else None
        try:
            function_name = tool_call["function_name"]
            arguments = tool_call["arguments"]

            logger.info("Executing tool: {function_name} with arguments: {arguments}")
            if self.trace:
            ...
            if not tool_fn:
                logger.error("Tool function '{function_name}' not found in registry")
                if span:
                    span.end(status_message="tool_not_found", level="ERROR")
                return ToolResult(success=False, output="Tool function '{function_name}' not found")

            logger.debug("Found tool function for '{function_name}', executing...")
            result = await tool_fn(**arguments)
            logger.info("Tool execution complete: {function_name} -> {result}")
            if span:
                span.end(status_message="tool_executed", output=result)
            return result
        except Exception as e:
            logger.error("Error executing tool {tool_call['function_name']}: {str(e)}", exc_info=True)
            if span:
                span.end(status_message="tool_execution_error", output="Error executing tool: {str(e)}", level="ERROR")
            return ToolResult(success=False, output="Error executing tool: {str(e)}")
    ...
    async def _execute_tools(
        ...
        """
        logger.info("Executing {len(tool_calls)} tools with strategy: {execution_strategy}")
        if self.trace:
            self.trace.event(name="executing_tools_with_strategy", level="DEFAULT", status_message=("Executing {len(tool_calls)} tools with strategy: {execution_strategy}"))
        ...
        if execution_strategy == "sequential":
            ...
            logger.warning("Unknown execution strategy: {execution_strategy}, falling back to sequential")
            return await self._execute_tools_sequentially(tool_calls)
    ...
    async def _execute_tools_sequentially(self, tool_calls: List[Dict[str, Any]]) -> List[Tuple[Dict[str, Any], ToolResult]]:
        ...
        try:
            ...
            logger.info("Executing {len(tool_calls)} tools sequentially: {tool_names}")
            if self.trace:
                self.trace.event(name="executing_tools_sequentially", level="DEFAULT", status_message=("Executing {len(tool_calls)} tools sequentially: {tool_names}"))

            results = []
            for index, tool_call in enumerate(tool_calls):
                tool_name = tool_call.get('function_name', 'unknown')
                logger.debug("Executing tool {index+1}/{len(tool_calls)}: {tool_name}")

                try:
                    result = await self._execute_tool(tool_call)
                    results.append((tool_call, result))
                    logger.debug("Completed tool {tool_name} with success={result.success}")

                    # Check if this is a terminating tool (ask or complete)
                    if tool_name in ['ask', 'complete']:
                        logger.info("Terminating tool '{tool_name}' executed. Stopping further tool execution.")
                        if self.trace:
                            self.trace.event(name="terminating_tool_executed", level="DEFAULT", status_message=("Terminating tool '{tool_name}' executed. Stopping further tool execution."))
                        break  # Stop executing remaining tools

                except Exception as e:
                    logger.error("Error executing tool {tool_name}: {str(e)}")
                    if self.trace:
                        self.trace.event(name="error_executing_tool", level="ERROR", status_message=("Error executing tool {tool_name}: {str(e)}"))
                    error_result = ToolResult(success=False, output="Error executing tool: {str(e)}")
                    results.append((tool_call, error_result))

            logger.info("Sequential execution completed for {len(results)} tools (out of {len(tool_calls)} total)")
            if self.trace:
                self.trace.event(name="sequential_execution_completed", level="DEFAULT", status_message=("Sequential execution completed for {len(results)} tools (out of {len(tool_calls)} total)"))
            return results

        except Exception as e:
            logger.error("Error in sequential tool execution: {str(e)}", exc_info=True)
            ...
            error_results = [(tool, ToolResult(success=False, output="Execution error: {str(e)}"))
    ...
    async def _execute_tools_in_parallel(self, tool_calls: List[Dict[str, Any]]) -> List[Tuple[Dict[str, Any], ToolResult]]:
        ...
        try:
            tool_names = [t.get('function_name', 'unknown') for t in tool_calls]
            logger.info("Executing {len(tool_calls)} tools in parallel: {tool_names}")
            if self.trace:
                self.trace.event(name="executing_tools_in_parallel", level="DEFAULT", status_message=("Executing {len(tool_calls)} tools in parallel: {tool_names}"))
            ...
            for i, (tool_call, result) in enumerate(zip(tool_calls, results)):
                if isinstance(result, Exception):
                    logger.error("Error executing tool {tool_call.get('function_name', 'unknown')}: {str(result)}")
                    if self.trace:
                        self.trace.event(name="error_executing_tool", level="ERROR", status_message=("Error executing tool {tool_call.get('function_name', 'unknown')}: {str(result)}"))
                    # Create error result
                    error_result = ToolResult(success=False, output="Error executing tool: {str(result)}")
                    processed_results.append((tool_call, error_result))
                else:
                    processed_results.append((tool_call, result))

            logger.info("Parallel execution completed for {len(tool_calls)} tools")
            if self.trace:
                self.trace.event(name="parallel_execution_completed", level="DEFAULT", status_message=("Parallel execution completed for {len(tool_calls)} tools"))
            return processed_results

        except Exception as e:
            logger.error("Error in parallel tool execution: {str(e)}", exc_info=True)
            if self.trace:
                self.trace.event(name="error_in_parallel_tool_execution", level="ERROR", status_message=("Error in parallel tool execution: {str(e)}"))
            # Return error results for all tools if the gather itself fails
            return [(tool_call, ToolResult(success=False, output="Execution error: {str(e)}"))
    ...
    async def _add_tool_result(
        ...
        try:
            ...
            if assistant_message_id:
                ...
                logger.info("Linking tool result to assistant message: {assistant_message_id}")
                if self.trace:
                    self.trace.event(name="linking_tool_result_to_assistant_message", level="DEFAULT", status_message=("Linking tool result to assistant message: {assistant_message_id}"))
            ...
            if "id" in tool_call:
                ...
                logger.info("Formatted tool result content: {content[:100]}...")
                if self.trace:
                ...
                logger.info("Adding native tool result for tool_call_id={tool_call['id']} with role=tool")
                if self.trace:
                    self.trace.event(name="adding_native_tool_result_for_tool_call_id", level="DEFAULT", status_message=("Adding native tool result for tool_call_id={tool_call['id']} with role=tool"))
            ...
            if is_mcp_tool:
                ...
                logger.info("Adding MCP tool result with simplified format for LLM visibility")
                if self.trace:
            ...
            logger.error("Error adding tool result: {str(e)}", exc_info=True)
            if self.trace:
            ...
            try:
                ...
                logger.error("Failed even with fallback message: {str(e2)}", exc_info=True)
                if self.trace:
    ...
    def _create_structured_tool_result(self, tool_call: Dict[str, Any], result: ToolResult, parsing_details: Optional[Dict[str, Any]] = None):
        ...
        #     summary = "Tool '{xml_tag_name}' {status}. Output: {summary_output}"
        # else:
        #     status = "completed successfully" if structured_result_v1["tool_execution"]["result"]["success"] else "failed"
        #     summary = "Function '{function_name}' {status}. Output: {summary_output}"
        ...
        if self.is_agent_builder:
            ...
            logger.info("Returning sumnary for data provider call: {summary}")
            return summary
    ...
    def _format_xml_tool_result(self, tool_call: Dict[str, Any], result: ToolResult) -> str:
        ...
        if "xml_tag_name" in tool_call:
            ...
            return "<tool_result> <{xml_tag_name}> {str(result)} </{xml_tag_name}> </tool_result>"

        # Non-XML tool, just return the function result
        function_name = tool_call["function_name"]
        return "Result for {function_name}: {str(result)}"
    ...
    async def _yield_and_save_tool_started(self, context: ToolExecutionContext, thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        content = {
            ...
            "message": "Starting execution of {tool_name}", "tool_index": context.tool_index,
    ...
    async def _yield_and_save_tool_completed(self, context: ToolExecutionContext, tool_message_id: Optional[str], thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        if context.function_name in ['ask', 'complete']:
            ...
            logger.info("Marking tool status for '{context.function_name}' with termination signal.")
            if self.trace:
                self.trace.event(name="marking_tool_status_for_termination", level="DEFAULT", status_message=("Marking tool status for '{context.function_name}' with termination signal."))
        # <<< END ADDED >>>
    ...
    async def _yield_and_save_tool_error(self, context: ToolExecutionContext, thread_id: str, thread_run_id: str) -> Optional[Dict[str, Any]]:
        ...
        content = {
            ...
            "function_name": context.function_name, "xml_tag_name": context.xml_tag_name,
            "message": "Error executing tool {tool_name}: {error_msg}",
