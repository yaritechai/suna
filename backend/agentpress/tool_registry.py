...
class ToolRegistry:
    ...
    def register_tool(self, tool_class: Type[Tool], function_names: Optional[List[str]] = None, **kwargs):
        ...
        """
        logger.debug("Registering tool class: {tool_class.__name__}")
            ...
            if function_names is None or func_name in function_names:
                for schema in schema_list:
                    ...
                    if schema.schema_type == SchemaType.XML and schema.xml_schema:
                        ...
                        registered_xml += 1
                        logger.debug("Registered XML tag {schema.xml_schema.tag_name} -> {func_name} from {tool_class.__name__}")

        logger.debug("Tool registration complete for {tool_class.__name__}: {registered_openapi} OpenAPI functions, {registered_xml} XML tags")
    ...
    def get_available_functions(self) -> Dict[str, Callable]:
        ...
        logger.debug("Retrieved {len(available_functions)} available functions")
    ...
    def get_xml_tool(self, tag_name: str) -> Dict[str, Any]:
        ...
        if not tool:
            logger.warning("XML tool not found for tag: {tag_name}")
        return tool
    ...
    def get_openapi_schemas(self) -> List[Dict[str, Any]]:
        ...
        logger.debug("Retrieved {len(schemas)} OpenAPI schemas")
        return schemas
    ...
    def get_xml_examples(self) -> Dict[str, str]:
        ...
        logger.debug("Retrieved {len(examples)} XML examples")
        return examples
