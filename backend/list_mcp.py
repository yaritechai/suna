        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for test_server in test_servers:
                    ...
                    details_url = "https://registry.smithery.ai/servers/{encoded_name}"
                    ...
                    if response.status_code == 200:
                        ...
                        print("  Display Name: {server_details.get('displayName')}")
                        print("  Description: {server_details.get('description', '')[:100]}...")
                        print("  Homepage: {server_details.get('homepage')}")
                        print("  Is Deployed: {server_details.get('isDeployed', False)}")

                        tools = server_details.get("tools", [])
                        print("  Available tools ({len(tools)}):")
                        for tool in tools[:5]:  # Show first 5 tools
                            print("    - {tool.get('name')}: {tool.get('description', 'No description')[:60]}...")
                    else:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print("  Total servers: {data.get('total', 0)}")
                        print("  Categories: {data.get('categoryCount', 0)}")
                        if data.get('categorized'):
                            for cat, servers in list(data['categorized'].items())[:3]:
                                print("  - {cat}: {len(servers)} servers")
                    else:
                        print("✗ API endpoint failed: {response.status_code}")
                except Exception as e:
                    print("✗ Could not reach API endpoint (is the server running?): {e}")
